...
namespace DataStructures.Graph.AdjancencySet
{
    public class WeightedDiGraph<T, TW> : IDiGraph<T> where TW : IComparable
    {
        ...
        public void RemoveVertex(T key)
        {
            ...
            {
               vertices[vertex.Key.Key].OutEdges.Remove(vertices[key]);
            }
            foreach (var vertex in vertices[key].InEdges)
            {
                vertices[vertex.Key.Key].InEdges.Remove(vertices[key]);
            }
            vertices.Remove(key);
        }
        ...
        private class WeightedDiGraphVertex<T, TW> : IDiGraphVertex<T> where TW : IComparable
        {
            ...
            IEnumerable<IDiGraphEdge<T>> IDiGraphVertex<T>.OutEdges => OutEdges.Select(x => new DiEdge<T, TW>(x.Key, x.Value));
            IEnumerable<IDiGraphEdge<T>> IDiGraphVertex<T>.InEdges => InEdges.Select(x => new DiEdge<T, TW>(x.Key, x.Value));
            ...
            public IEnumerable<IGraphEdge<T>> Edges => OutEdges.Select(x => new Edge<T, TW>(x.Key, x.Value));
